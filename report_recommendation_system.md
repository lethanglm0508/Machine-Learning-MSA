...
graph TD
    ...
    C --> D2[Tính thống kê Collaborative từ tập Train & User Profiles]
...
- **Đầu vào (Input):**
  ...
  - Metadata sản phẩm: Tiêu đề (`meta_title`), Số lượng đánh giá toàn cầu (`rating_number`).
...
> [!IMPORTANT]
> **Quyết định Thiết kế Tránh Rò Rỉ Dữ Liệu và Mismatch Thực Tế:**
> 1. **Bỏ các trường `verified_purchase` và `helpful_vote`:** Các trường này chỉ biết được *sau khi* người dùng mua và viết nhận xét (review). Trong hệ thống gợi ý thực tế, trước khi recommend, chúng ta chưa có tương tác này. Do đó, việc dùng chúng làm đặc trưng là không hợp lý.
> 2. **Bỏ trường `average_rating` từ Metadata gốc:** Điểm đánh giá trung bình của sản phẩm trên Amazon được tính dựa trên toàn bộ dữ liệu (cả tập Train và Test), sử dụng nó sẽ gây ra rò rỉ thông tin đích (Target leakage). Thay vào đó, chúng ta tính toán đặc trưng `item_avg_rating` thuần túy từ tập Train.
...
Các điểm dữ liệu (tạp chí) trên biểu đồ 2D được tô màu dựa trên số lượng đánh giá toàn cầu (`rating_number` thu được từ metadata của Amazon).
...
*Nhận xét:* Biểu đồ t-SNE tạo ra các cụm sản phẩm rõ ràng hơn so với PCA, thể hiện các sản phẩm có tiêu đề tương tự nhau về mặt ngữ nghĩa (ví dụ: các tạp chí thể thao, thời trang, chuyên ngành kỹ thuật) được gom nhóm lại với nhau.
...
1. **Tải dữ liệu từ tệp JSONL gốc:** 2.59 giây.
2. **Tiền xử lý & lọc thưa:** 1.76 giây.
3. **Trích xuất đặc trưng & giảm chiều TF-IDF PCA:** 16.21 giây.
4. **Trực quan hóa dữ liệu (PCA/t-SNE):** 11.37 giây.
5. **Huấn luyện mô hình hồi quy (Ridge):** **0.0093 giây** (Cực kỳ nhanh chóng nhờ số chiều đặc trưng thấp $d=16$).
6. **Dự báo & Đánh giá xếp hạng Top-10 cho 1,266 người dùng:** 16.78 giây (sử dụng vector hóa dữ liệu để dự báo song song trên toàn bộ ứng viên chưa tương tác).
- **Tổng thời gian chạy toàn bộ luồng:** **48.72 giây**.

#### Độ phức tạp tính toán lý thuyết:
- **Huấn luyện Ridge Regression:** $\mathcal{O}(N \cdot d^2 + d^3)$ với $N = 5001$ (số tương tác train), $d = 16$ (số đặc trưng). Do $d \ll N$, việc huấn luyện diễn ra cực kỳ nhanh.
...
| **Ridge Regression ($\alpha = 10.0$)** | **0.9749** | **0.6459** |
| *User Mean Baseline* | *0.9678* | *0.5506* |

*Nhận xét về kết quả dự đoán điểm:*
- Cả Linear và Ridge Regression đều cải thiện sai số rất rõ rệt so với Global Mean Baseline (RMSE giảm từ `1.0896` xuống `0.9782` và `0.9749`). Điều này khẳng định việc trích xuất và kết hợp các đặc trưng có giá trị thông tin cao.
...
- **Tuy nhiên, mô hình hồi quy chưa vượt qua được User Mean Baseline (RMSE = 0.9678).** Điều này hoàn toàn hợp lý về mặt bản chất hệ thống gợi ý: xu hướng điểm đánh giá lịch sử của từng người dùng (User Bias) là tín hiệu mạnh nhất để dự đoán điểm số rating. Do dữ liệu cực kỳ thưa thớt (99.36% ô trống) và mối tương quan giữa người dùng - sản phẩm có thể mang tính phi tuyến phức tạp, mô hình tuyến tính học các trọng số toàn cục tĩnh vẫn bị giới hạn nhất định trong việc tối ưu hóa điểm số.
...
- Kết quả thực nghiệm cho thấy Popularity Baseline vượt trội hoàn toàn so với mô hình Ridge Regression trên các chỉ số xếp hạng (Precision@10 cao gấp 12 lần, NDCG@10 cao gấp 13 lần).
- **Nguyên nhân vật lý:**
...
- Đặc trưng mô hình đã được tinh chỉnh tối ưu: loại bỏ hoàn toàn các trường dữ liệu rò rỉ (`average_rating` metadata) và các trường phi thực tế trước khi khuyến nghị (`verified_purchase`, `helpful_vote`), mang lại tính đúng đắn về khoa học dữ liệu.
...
- Thực nghiệm chỉ ra các kết luận trung thực và thực tế: Mô hình Hồi quy Tuyến tính cải thiện rõ rệt so với Global Mean Baseline nhưng vẫn chưa vượt qua được User Mean Baseline do dữ liệu thưa và các tương tác phi tuyến. Đồng thời, khả năng xếp hạng gợi ý Top-N trên dữ liệu cực thưa vẫn chưa vượt qua được mô hình cơ sở phổ biến (Popularity Baseline).
